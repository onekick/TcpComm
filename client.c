...
int main(int argc, char *argv[]) // 쉘에서 ip와 port 번호를 받는다.
{
 int sock;  // 서버에 접속할 소켓
 ...
 if(sock==-1) // 안만들어 지면
  error_handling("socket() error"); // 에러표시후 종료
 ...
 while(1) // 연결되고 나면 무한루프 진입, 언제 끝나느냐?
 {
  ...
  fgets(message, BUF_SIZE, stdin); // 사용자로 부터 메시지를 입력받는다.
  ...
  message[str_len]=0; // 보낼때 NULL문자 안보내서, 받았을때 넣어 줘야 한다.
